...
#  classNum 类别总数(含背景)
#  colorDict_GRAY 颜色字典
#  resize_shape resize大小                                                    # colorDict_GRAY
def trainGenerator(batch_size, train_image_path, train_label_path, classNum, colorDict_GRAY, resize_shape=None):
    ...
    #  无限生成数据
    while True:
        img_generator = np.zeros((batch_size, img.shape[0], img.shape[1], img.shape[2]), np.uint32)
        label_generator = np.zeros((batch_size, img.shape[0], img.shape[1]), np.uint8)
        if resize_shape is not None:
            img_generator = np.zeros((batch_size, resize_shape[0], resize_shape[1], resize_shape[2]), np.uint32)
        ...
        for j in range(batch_size):
            ...
            if resize_shape is not None:
                label = np.resize(label, (resize_shape[0], resize_shape[1]))
            label_generator[j] = label                                                           # colorDict_GRAY
